...
class ReviewLeaveDatabase {
  ...
  Future<dynamic> listOfAllLeaves() async {
    DataSnapshot dataSnapshot = await _databaseReference.child("leaves").once();
  ...
  Future<List<Leave>> getLeaveListBasedOnUID(String uid) async {
    DataSnapshot dataSnapshot =
        await _databaseReference.child("Managers").child(uid).once();
    List<Leave> result = [];
  ...
  Future<void> approveLeave(
    ...
    await _databaseReference.child("users").child(uid).child("leaves").update({
      getLeaveType(leaveType): (dataSnapshot.value - days),
    });
